...
const parts = original.split('export function planField(');
let before = parts[0];
let afterPlanField = parts[1];
// Find the end of planField. It's the first top-level '}\n'
const planFieldEnd = afterPlanField.indexOf('}\n\n');
let after = afterPlanField.substring(planFieldEnd + 3);

// wait, valueToInteraction is below planField. We need it.
// Wait, is valueToInteraction below or above planField?
// Let's just append the new planField at the end.
// And remove the old planField block entirely.

const valueToInteractionIdx = afterPlanField.indexOf('export function valueToInteraction(');
if (valueToInteractionIdx !== -1) {
  after = afterPlanField.substring(valueToInteractionIdx);
} else {
  after = ''; // If it's above, we handled it wrong. Let's hope it's below.
}

let code = before + '\n' + after;

// Now add imports to top
