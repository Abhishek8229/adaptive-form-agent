...
// Replace the old planField
const beforePlanField = original.substring(0, original.indexOf('export function planField('));

// We don't need afterPlanField, because planField is the LAST thing in the file.
// Let's verify that.

let code = beforePlanField;
...
import type { FieldVisualContext, FormPage } from '../shared/types';
`;
...
export async function planField(
  ...
  if (visionProvider) {
    ...
    if (visualCtx?.screenshot) {
      const candidateKeys = Object.keys(profile);
      const visionRes = await visionProvider.extractFieldSemantics({
        screenshot: visualCtx.screenshot.dataUrl,
        nearbyText: visualCtx.nearbyText,
        candidateKeys,
      });
