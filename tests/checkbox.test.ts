...
test('checkbox array matching uses option value', () => {
  ...
  const p = planField(field, { frameworks: ['react', 'vue'] });
  assert.equal(p.ok, true);
  if (p.ok) assert.equal(p.request.kind, 'check', 'Should match by option value');
});

test('required consent checkboxes auto-check', () => {
  const field = makeCheckbox({
    name: 'terms',
    label: 'I agree to the terms',
    required: true
  });
  const p = planField(field, { somethingElse: 'value' }); // No matching profile key
  assert.equal(p.ok, true);
  if (p.ok) {
